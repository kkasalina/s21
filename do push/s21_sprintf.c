...
int print_format(s_info *ints, const char *format, int temp) {
  while (check_format(format, temp)) {
    ...
    if (format[temp] == 'h') ints->flag_h++;  // 88888888
    if (format[temp] == 'l') ints->flag_l++;  // 88888888
    if (format[temp] == 'L') ints->flag_l++;  // 88888888
    temp++;
  }
...
void ftoa(long double n, char *res, s_info *ints) {
  ...
  if (ints->precision != 0) {
    ...
    fpart = fpart / 10.0f;
    intToStr((long long int)fpart, res + i + 1, ints->precision);
  }
}
...
int check_format(const char *format, int temp) {
  if (format[temp] != 'c' && format[temp] != 's' && format[temp] != 'p' &&
      ...
      format[temp] != 'h' && format[temp] != 'l' && format[temp] != 'L' && // 8888888888
...
int check_format_letter(const char *format, int temp) {
  if (format[temp] != 'c' && format[temp] != 's' && format[temp] != 'p' &&
      format[temp] != 'd' && format[temp] != 'i' && format[temp] != 'f' &&
      format[temp] != 'g' && format[temp] != 'G' && format[temp] != 'e' &&
      format[temp] != 'h' && format[temp] != 'l' && format[temp] != 'L' && // 888888888
      format[temp] != 'n' && format[temp] != 'u' && format[temp] != 'x' && // 888888888
...
int check_format_letter_2(const char *format, int temp) {
  if (format[temp] != 'c' && format[temp] != 's' && format[temp] != 'p' &&
      ...
      format[temp] != 'g' && format[temp] != 'G' && format[temp] != 'e' &&
      format[temp] != 'u' && format[temp] != 'x' && format[temp] != 'X' &&
      format[temp] != 'n' && format[temp] != 'o' && format[temp] != '%' && // 888888888
